...
class nnUNetPredictor(object):
    def __init__(self,
        ...
        ##检擦设备是否使用GPU，如是赋值print详细信息：（设备信息和操作存储在是实例变量中）
        if device.type == 'cuda':
            self.verbose = verbose
            self.verbose_preprocessing = verbose_preprocessing
            self.allow_tqdm = allow_tqdm
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        network = trainer_class.build_network_architecture(
            configuration_manager.network_arch_class_name,#网络架构类型名
            configuration_manager.network_arch_init_kwargs,#初始化参数
            configuration_manager.network_arch_init_kwargs_req_import,#所需导入的信息
            num_input_channels,#通道数量
            plans_manager.get_label_manager(dataset_json).num_segmentation_heads,#用于分割的头部数量
            enable_deep_supervision=False#是否启用深度监督
        )
