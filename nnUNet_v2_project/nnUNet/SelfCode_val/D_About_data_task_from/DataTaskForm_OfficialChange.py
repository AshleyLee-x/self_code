...
if __name__ == "__main__":
    ...
    dataset_name = 'Dataset888_cells'#需要修改本句！！
    ...
    train_source = join(source, 'train')##改了！！
    test_source = join(source, 'test')##改了！！！

    with multiprocessing.get_context("spawn").Pool(8) as p:

        # not all training images have a segmentation
        valid_ids = subfiles(join(train_source, 'labels'), join=False, suffix='png')##改了！！！
        ...
        for v in valid_ids:
            r.append(
                p.starmap_async(
                    ...
                    ((
                         join(train_source, 'images', v),##改了！！！
                         join(train_source, 'labels', v),##改了！！！
        ...
        # test set
        valid_ids = subfiles(join(test_source, 'labels'), join=False, suffix='png')##改了！！！
        for v in valid_ids:
            r.append(
                p.starmap_async(
                    load_and_convert_case,
                    ((
                         join(test_source, 'images', v),##改了！！！
                         join(test_source, 'labels', v),##改了！！！
    ...
    generate_dataset_json(join(nnUNet_raw, dataset_name), {0: 'R', 1: 'G', 2: 'B'}, {'background': 0, 'cell_nucleus': 1},
                          num_train, '.png', dataset_name=dataset_name)##改了！！！
    ##histocyte:组织细胞
